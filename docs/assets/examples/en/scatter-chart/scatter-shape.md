...
# Scatter Chart with Various Shapes

Differentiate various point types through the shape visual channel

## Key Configuration

- The `shapeField` attribute is declared as the size field
- The `shape` visual channel for point shapes is configured as a `scale` type, achieving the mapping of shape types based on specific fields
