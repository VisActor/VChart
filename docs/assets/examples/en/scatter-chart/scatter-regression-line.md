...
# Scatter Plot with Regression Line Support

## Key Configurations

- Register the regression line extension using `registerRegressionLine()`
- Use `appendScatterRegressionLineConfig(spec, [...])` to configure multiple regression lines (such as polynomial regression, linear regression, etc.)
- You can set the regression line type (`type: 'linear' | 'logistic' | 'lowess' | 'polynomial'`), color, degree (for polynomial), confidence interval style, label, and more
...
{
type: 'polynomial', // 支持 4 中类型 'linear' | 'logisitc' | 'lowess' | 'polynomial'
polynomialDegree: 3,
color: 'red',
line: {
style: {
