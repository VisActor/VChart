...
order: 1-3
cover: https://lf9-dp-fe-cms-tos.byteorg.com/obj/bit-cloud/vchart/preview/area-chart/null-value-area.png
option: areaChart
---

# Area chart with null values

An area chart with null values in the data. Avoid interpolating missing values in the original data set to show the data's original form. This example demonstrates the results of Australia's quarterly job vacancy survey, with some data missing from 2008-2009.

## Key configuration

The `invalidType` property is set for `null`, `undefined`, and other non-compliant data point connection methods:

- 'break' means to break at that data point
- 'link' means to ignore that point and keep going
