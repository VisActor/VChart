...
title: 扰动散点图
order: 9-2
cover: http://tosv.byted.org/obj/bit-cloud/vchart/preview/scatter-chart/jitter-scatter.png
option: scatterChart
---

# 扰动散点图

扰动散点图在数据点的位置上添加了一些随机扰动，以避免数据点重叠在一起。

这种图形通常用于显示两个变量之间的关系，其中一个变量是连续的，另一个变量是分类的。在这个示例中,cylinders 是分类的, horsepower 是连续的. 用来观察不同气缸数与马力之间的关系.

扰动散点图可以帮助观察者更清楚地看到数据点的分布和趋势，特别是在数据点密集的区域。
