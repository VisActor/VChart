...
旭日图在层次过深时, 每一个扇区的面积会非常小, 导致标签非常密集, 不利于可视化分析, 在这种情况下, 旭日图支持配置`labelAutoVisible`来自适应显示标签.

> 本例中数据来源于一个可视化工具包 Flare 源码的文件大小层次数据。

## 关键配置

- `labelAutoVisible` 自动隐藏标签
  - `enable` 功能开关
  - `circumference` 周长阈值, 扇区周长低于此值时则隐藏扇区的标签, 反之显示标签.
