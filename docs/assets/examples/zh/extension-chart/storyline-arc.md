...
本示例沿用 `packages/vchart-extension/__tests__/runtime/browser/test-page/storyline.ts` 中的 `arc` case，保留原始图片、数据和配色，画布宽高自适应容器。
...
- `layout: { type: 'arc', direction: 'up' }`：穹顶布局。
- `titleImage.image`：底部世界杯主题图。
