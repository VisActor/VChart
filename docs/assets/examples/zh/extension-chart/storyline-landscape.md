...
本示例沿用 `packages/vchart-extension/__tests__/runtime/browser/test-page/storyline.ts` 中的 `landscape` case，保留原始图片、数据和配色，画布宽高自适应容器。
...
- `layout: 'landscape'`：沿横向曲线排列节点。
- `line`：保留测试 case 的连线配置。
- `image.showBackground: false`：隐藏主图背景装饰。
