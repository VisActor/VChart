...
order: 12-6
cover: https://tosv.byted.org/obj/bit-cloud/57a706137957fce7388f3ab02.png
option: sankeyChart
---
...
桑基图的数据支持三种形式：
- 节点数据与边数据组成的字典形式，例如`{nodes: [{nodeName: "xxx"}, {nodeName: "xxx"}], links: [{ source: 0, target: 1, value: xxx }]}`, 对应于`基础桑基图`示例。
- 节点数据与边数据的json数据组成的数组形式，例如`[{id: 'nodes',values: [{nodeName: "xxx"}, {nodeName: "xxx"}]}, {id: 'links',values: [{ source: 0, target: 1, value: xxx }]}]`，对应于该示例。
- 层级数据格式，例如`nodes: [{values: xxx, name: xx, children: [{values: xxx, name: xx}, {values: xxx, name: xx}]}]`, 对应于`层级桑基图`示例。
