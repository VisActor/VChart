...
`scrollBar` 可以通过`roamDrag`和`roamZoom`配置来实现控制区域内的画布缩放和拖拽。

## 关键配置

- `roamDrag` 属性声明为拖拽配置
- `roamDrag.rate` 属性声明为拖拽速率，范围`[0, 1]`
- `roamDrag.reverse` 属性声明为拖拽方向与滚动条移动方向是否相反

- `roamZoom` 属性声明为缩放配置
- `roamZoom.rate` 属性声明为缩放速率，范围`[0, 1]`
- `roamDrag.focus` 属性声明为是否聚焦缩放，开启时, 默认以鼠标位置为中心缩放，关闭时，默认以画布中心缩放
