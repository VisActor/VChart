...
正如上文所说，散点图用点的位置表示两个变量之间的关系，点的大小或颜色可以用来区分类别。而在VChart中，可以通过 `scatterChart.sizeField`指定散点的大小，从而以此表示变量的数值大小差异，这种指定散点大小的可视化表达方式也叫气泡图。
...
- `scatterChart.xField`: 分类/数值字段，映射图元的x坐标
- `scatterChart.yField`: 分类/数值字段，映射图元的y坐标
...
- 更多组件配置见[VChart scatterChart配置](../../../option/scatterChart)
...
### 数据
- 一个`离散`/ `数值` 字段，如: `product`，映射图元x坐标   
- 一个`离散`/ `数值`字段，如: `sales` ，映射图元y坐标
