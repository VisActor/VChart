...
```typescript
interface ILiquidOutlineSpec extends ISymbolMarkSpec {
  // Liquid outline-specific properties
  lineWidth: number; // Stroke width (required)

  // Inherited from ISymbolMarkSpec
  dx?: number; // X-direction offset
  dy?: number; // Y-direction offset
  size?: number | number[]; // Size (outer circle radius)
  shape?: ShapeType | string; // Shape type
  symbolType?: ShapeType | string; // Symbol type
...
```typescript
type ShapeType = SymbolType;
```

ShapeType 从`@visactor/vrender-core`导入，支持多种几何形状包括圆形、矩形、三角形等。
