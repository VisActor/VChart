...
class TestStageAnimate extends AStageAnimate<any> {
  ...
  protected afterStageRender(stage: any, canvas: HTMLCanvasElement): HTMLCanvasElement | void | null | false {
    console.time('ParticleDisintegration');
    ...
    console.timeEnd('ParticleDisintegration');
    return result;
  }
}
...
const currentParticleEffect = 'explode';
const currentParticleCount = 4000;
const currentParticleSize = 10;
const currentForceStrength = 1.5;
...
- 'vortex': 漩涡退场效果 - 粒子以螺旋方式向外扩散离开canvas
...
const run = () => {
  ...
  // 6. 重新生成粒子按钮
  const regenerateParticlesButton = document.createElement('button');
  regenerateParticlesButton.innerHTML = '重新生成粒子动画';
  regenerateParticlesButton.style.backgroundColor = '#3F51B5';
  regenerateParticlesButton.style.color = 'white';
  regenerateParticlesButton.style.margin = '10px';
  regenerateParticlesButton.style.padding = '10px';
