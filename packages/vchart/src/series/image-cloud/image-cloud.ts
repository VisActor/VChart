      ...
      onUpdateMaskCanvas: ((inputImage?: HTMLCanvasElement, maskImage?: HTMLCanvasElement) => {
        if (inputImage && this._maskMark) {
          this._maskMark
            ...
            .getAllElements()
            .forEach(element => {
              element.getGraphicItem().setAttribute('background', inputImage);
            });
        }
        if (maskImage && (this._spec.layoutConfig as GridLayoutConfig)?.placement === 'masked') {
          this._rootMark.getProduct().getGroupGraphicItem().setAttribute('background', maskImage);
        }
      }).bind(this)
    };
  }
