  ...
  implements ICrossHair
{
  ...
  protected _getAxisInfoByField<T = IAxis>(field: 'x' | 'y' | 'category' | 'value') {
    ...
    bindingAxesIndex.forEach(idx => {
      ...
      regions.forEach(r => {
        const { x: regionStartX, y: regionStartY } = r.getLayoutPositionWidthIndent();
        x1 = Math.min(x1, regionStartX - sx);
        y1 = Math.min(y1, regionStartY - sy);
        x2 = Math.max(x2, regionStartX + r.getLayoutRectWidthIndent().width - sx);
        y2 = Math.max(y2, regionStartY + r.getLayoutRectWidthIndent().height - sy);
      });
  ...
  onLayoutEnd(ctx: IModelLayoutOption): void {
    ...
    this.setLayoutRect(region.getLayoutRectWidthIndent());
    this.setLayoutStartPosition(region.getLayoutPositionWidthIndent());
