import { type TableData, type AggregateMethod, AggregateType } from '../types';
import { of } from '../pipe';
...
test('select grouped data without aggregate', () => {
  ...
  expect(result1).toEqual([{ subject: 'Sword' }, { subject: 'System' }]);
  ...
  expect(result2).toEqual([{ subject: 'Sword' }, { subject: 'Sword' }, { subject: 'System' }, { subject: 'System' }]);
...
test('select with custom aggregation method', () => {
  const customNameAggregate: AggregateMethod = ({ values }) => {
    return (values as string[]).map(name => name.slice(0, 1)).join(' ');
  };
  ...
  expect(result).toEqual([{ 'Initial Letters': 'A E K F C A' }]);
});
