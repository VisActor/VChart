import { type GroupedData, AggregateType, FilterNodeType, FilterOperator } from '../../types';
import { of } from '../../pipe';
        ...
        conditions: [
          {
            type: FilterNodeType.Condition,
            aggregate: ({ group }) =>
              Array.from(new Set(group.rows.map(row => row.age as number))).reduce((sum, age) => sum + age, 0),
            operator: FilterOperator.GreaterThan,
            value: 50
          },
          {
