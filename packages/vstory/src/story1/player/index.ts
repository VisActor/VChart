...
import { IAction, IChapterRoleLink, IChapterSpec } from '../interface';
...
export class Player implements IPlayer {
  ...
  addChapter(
    ...
    const e: { role: IRole; actions: IAction[] }[] = [];
    c.steps.forEach(item => {
      e.push({
        role: roles[(item as any).roleId],
  ...
  tickTo(t: number) {
    ...
    this._currChapter.roles.forEach(({ actions, role }) => {
    ...
    this._currChapter.roles.forEach(({ role }) => {
      role.hide;
    });
