...
export class DataTempTransformBase implements IDataTempTransform {
  ...
  protected _createNextTemp(temp: string) {
    ...
    this._nextTemp = StoryFactory.createChartTemp(temp, this._specProcess.getRoleSpec());
    return !!this._nextTemp;
  }
  ...
  protected _checkEnable(temp: IVisactorTemp) {
    ...
    if (temp.checkDataEnable(this._specProcess.getRoleSpec().options.data)) {
      return true;
    }
    return false;
  }
  ...
  getBaseSpec() {
    return this._specTemp.getSpec(this._specProcess.getRoleSpec().options.data, { role: this._role });
  }
