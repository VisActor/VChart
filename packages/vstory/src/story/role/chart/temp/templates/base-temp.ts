...
export abstract class BaseTemp implements IChartTemp {
  type: string;
  abstract getSpec(data: StandardData, info: DataInfo, ctx: { chart: RoleChart }, opt?: any): any;
   ...
   * @returns 返回更新后的图表配置
   */
  updateSpec(rawSpec: ISpec, config: IChartRoleSpec['config'], ctx: { chart: RoleChart }) {
    // transform spec 的过程
   ...
   * @returns 返回更新后的图表配置
   */
  transformSpec(rawSpec: ISpec, editorSpec: IChartRoleSpec['config'], ctx: { chart: RoleChart }) {
  ...
  processingVChart(ctx: { chart: RoleChart }) {}
}
