...
import { IComponentRoleSpec, IWidgetData } from '../..';
import { getLayoutFromWidget } from '../../../utils/layout';

export interface IGraphicConstructor {
  new (type: string, role: RoleComponent): Graphic;
}
...
export abstract class Graphic {
  ...
  protected _role: RoleComponent;
  protected _graphic: IGraphic<any>;
  protected _group: IGraphic<any>;

  constructor(type: string, role: RoleComponent) {
    this.type = type;
    this._role = role;
  }
  ...
  getGraphicAttribute(): IComponentRoleSpec['options']['graphic'] {
    return this._graphic?.attribute;
  }

  applyGraphicAttribute(graphicAttribute: IComponentRoleSpec['options']['graphic']): void {
    this._graphic.setAttributes(
  ...
  applyLayoutData(layoutData: Partial<IWidgetData>): void {
    this._graphic.setAttributes(
      this._transformAttributes({
        ...
        shapePoints: this._role.spec.options.shapePoints
      })
    );
  }
